...
class Mapping {
	...
	findRanges(view: MappingView): RangePair[] {
		var columns: RangePair[] = [];
	...
	findStart(root: Node, start: number, end: number): Range {
		...
		while (stack.length) {
			...
			if(found) {
				return this.findTextStartRange(found as Node, start, end);
			}
	...
	findEnd(root: Node, start: number, end: number): Range {
		...
		while (stack.length) {
			...
			if(found){
				return this.findTextEndRange(found as Node, start, end);
			}
