...
class Book {
	...
	constructor(url?: BookInput | BookOptions, options?: BookOptions) {
		...
		if(url) {
			this.open(url as BookInput, this.settings.openAs).catch((_error: unknown) => {
	...
	store(name: string): Store {
		...
		this.opened.then(() => {
			...
			// Substitute hook
			let substituteResources = (output: string, section: SectionLike) => {
			...
			(this.storage as StoreWithEvents).on("offline", () => {
			...
			(this.storage as StoreWithEvents).on("online", () => {
