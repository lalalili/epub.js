	...
	snapVerticalRlLogicalOffsetToTextBoundary(logicalOffset, maxScroll){
		...
		while ((node = walker.nextNode()) && inspected < 1000) {
			...
			for (const rect of Array.from(range.getClientRects())) {
				...
				let directDistance = Math.min(
					this.getVerticalRlRectDistanceToLogicalViewport(left, right, rightOriginLeft, rightOriginRight),
					this.getVerticalRlRectDistanceToLogicalViewport(left, right, leftOriginLeft, leftOriginRight)
				);
				let shiftedDistance = Math.min(
					this.getVerticalRlRectDistanceToLogicalViewport(shiftedLeft, shiftedRight, rightOriginLeft, rightOriginRight),
					this.getVerticalRlRectDistanceToLogicalViewport(shiftedLeft, shiftedRight, leftOriginLeft, leftOriginRight)
				);
		...
		let rightOriginSnap = evaluateBoundaryModel(
			rightOriginRawLeft + structuralLeftMask,
		...
		if (
			...
			edgeGuardPx > 0 &&
			structuralBleed > 1 &&
			Math.abs(boundaryShift - edgeGuardPx) <= 1
		) {
