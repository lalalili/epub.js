...
exports.stripScriptTagsFromContents = stripScriptTagsFromContents;
...
var _blob = require("../../platform/blob");
var _layout = require("../../platform/layout");
var _constants = require("../../utils/constants");
var _marksPane = require("marks-pane");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
const Defer = _async.defer;
const ContentsCtor = _contents.default;
...
class IframeView {
  ...
  constructor(section, options) {
    this.settings = (0, _collections.extend)({
      ignoreClass: "",
      axis: undefined,
    ...
    this.id = "epubjs-view-" + (0, _async.uuid)();
  ...
  create() {
    ...
    this.added = true;
    this.elementBounds = (0, _layout.bounds)(this.element);
  ...
  lock(what, width, height) {
    var elBorders = (0, _layout.borders)(this.element);
    var iframeBorders;
    if (this.iframe) {
      iframeBorders = (0, _layout.borders)(this.iframe);
    } else {
      iframeBorders = {
        width: 0,
        height: 0
  ...
  expand(force) {
    ...
    // Expand Horizontally
      ...
      const iframeWidth = Math.max(Number(this.iframe.clientWidth || 0), Number(this.iframe.getBoundingClientRect && this.iframe.getBoundingClientRect().width || 0), Number.parseFloat(this.iframe.style && this.iframe.style.width || "") || 0);
      if (iframeWidth <= 0 && visiblePageWidth > 0 && this.iframe.style && this.element && this.element.style) {
    ...
    if (measurementScrollContainer && measurementScrollContainer.scrollWidth > measurementScrollContainer.clientWidth) {
  ...
  reframe(width, height) {
    var size;
    if ((0, _types.isNumber)(width)) {
      this.element.style.width = width + "px";
      this.iframe.style.width = width + "px";
      this._width = Number(width);
    }
    if ((0, _types.isNumber)(height)) {
  ...
  load(contents) {
    ...
    if (this.settings.method === "blobUrl") {
      this.blobUrl = (0, _blob.createBlobUrl)(contents, "application/xhtml+xml");
  ...
  destroy() {
    ...
    if (this.blobUrl) {
      (0, _blob.revokeBlobUrl)(this.blobUrl);
    }
    if (this.displayed) {
      this.displayed = false;
