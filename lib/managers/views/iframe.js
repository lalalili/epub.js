...
class IframeView {
  ...
  render(request, show) {
    ...
    return this.sectionRender.then(function (contents) {
      ...
      let writingMode = this.settings.writingMode ? this.contents.writingMode(this.settings.writingMode) : this.contents.writingMode(); // Set the axis based on the flow and writing mode
  ...
  expand(force) {
    ...
    if (this.layout.name === "pre-paginated") {
      ...
      if (this.settings.flow === "paginated" && this.contents.writingMode && this.contents.writingMode() === "vertical-rl" && this.contents.verticalRlPageMetrics) {
        ...
        if (pageMetrics.effectivePageAdvance > 0) {
          ...
          if (this.layout.pageWidth !== pageWidth || this.layout.effectivePageAdvance !== pageAdvance || this.layout.delta !== pageAdvance || this.layout.pageBoundaryShift !== pageBoundaryShift || this.layout.edgeGuardPx !== edgeGuardPx) {
  ...
  onLoad(event, promise) {
    ...
    this.document = this.iframe.contentDocument;
    this.contents = new _contents.default(this.document, this.document.body, this.section.cfiBase, this.section.index, this.section.href);
