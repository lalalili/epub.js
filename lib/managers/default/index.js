  ...
  snapVerticalRlEdgeMaskWidths(widths, maxMask, limits = {}) {
    ...
    const snapRight = () => {
      ...
      for (const rect of rects) {
        ...
        if (rawRightStraddler && visibleInsideRawRight > edgeTolerance && right >= visibleInsideRawRight && rightMaxMask >= visibleInsideRawRight) {
          if (right > 0) {
            shrink = Math.min(shrink, -right);
          }
      ...
      if (shift !== 0) {
        let maxAllowedRight = shift > 0 ? expandBeyondPaintGuard ? maxMask : rightPaintGuardMax : rightMaxMask;
