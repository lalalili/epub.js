...
var _semanticCut = require("../../rendering/semantic-cut");
var _terminalContinuation = require("../../rendering/terminal-continuation");
var _terminalTail = require("../../rendering/terminal-tail");
...
const getVerticalRlTerminalTailSnapshot = observation => {
 ...
 */
const appendVerticalRlTerminalTrace = (event, detail) => {
  if (typeof window === "undefined" || !window.__EPUB_VRL_DEBUG__) {
    return;
  }
  let debugWindow = window;
  if (!Array.isArray(debugWindow.__EPUB_VRL_TERMINAL_TRACE__)) {
    debugWindow.__EPUB_VRL_TERMINAL_TRACE__ = [];
  }
  ...
  if (debugWindow.__EPUB_VRL_TERMINAL_TRACE__.length > 4000) {
    debugWindow.__EPUB_VRL_TERMINAL_TRACE__.splice(0, debugWindow.__EPUB_VRL_TERMINAL_TRACE__.length - 4000);
  }
};
class DefaultViewManager {
  ...
  display(section, target, options) {
    ...
    this.add(section, forceRight, requestWithCorrelation).then(function (view) {
      ...
      if (target) {
        ...
        if (this.layout.name === "reflowable" && this.layout.divisor > 1 && this.settings.axis === "horizontal" && offset.left >= this.container.scrollWidth) {
          this._pendingHorizontalTarget = {
            view,
            offset,
            width
          };
  ...
  resolveVerticalRlTerminalTailObservation() {
    ...
    if (!this.container || !iframe || !contentsDocument) {
      return {
        resolution: null,
        effectiveClip: null,
        frameIdentity: null,
    ...
    try {
      ...
      let effectiveClip = (0, _terminalTail.getVerticalRlEffectiveClip)(this.container);
      if (!effectiveClip) {
        return {
          resolution: null,
          effectiveClip: null,
          frameIdentity: (0, _terminalTail.getVerticalRlRuntimeIdentity)(iframe),
          documentIdentity: (0, _terminalTail.getVerticalRlRuntimeIdentity)(contentsDocument)
        };
      }
      return {
        resolution: (0, _terminalTail.resolveVerticalRlTerminalTail)({
  ...
  getVerticalRlLogicalPageOffsetCacheKey(_totalPages, maxScroll) {
    ...
    return (0, _pagination.getVerticalRlLogicalPageOffsetCacheKey)(this.getBaseGeometryPageCount(), maxScroll, contentWidth, visibleWidth, advance, edgeGuard);
  }
  ...
  getVerticalRlTerminalLayout() {
    ...
    const state = (0, _terminalContinuation.resolveVerticalRlTerminalContinuationWithObservation)(this._verticalRlTerminalLayouts.get(owner), key, {
      owner: this,
      section: owner,
      view,
  ...
  getVerticalRlSemanticCandidateObservation(view, physicalStart, physicalEnd) {
    ...
    if (!document || !viewWindow || !body || typeof walkerFactory !== "function" || !iframeRect || !Number.isFinite(physicalStart) || !Number.isFinite(physicalEnd) || physicalEnd <= physicalStart) {
      return {
        status: "unavailable",
    ...
    if (!Number.isFinite(clipLeft) || !Number.isFinite(clipRight) || !Number.isFinite(clipTop) || !Number.isFinite(clipBottom) || clipRight <= clipLeft || clipBottom <= clipTop) {
      return {
        status: "unavailable",
  ...
  createSemanticWindowDescriptor(location, sourceIdentity) {
    ...
    return {
      ...
      ...(view?.contents?.document && view?.iframe ? (() => {
        const semanticCut = (0, _semanticCut.captureSemanticCut)(view.contents.document, view.iframe, section.cfiBase);
        return semanticCut ? {
          semanticCut
        } : {};
      })() : {}),
  ...
  restoreSemanticWindowDescriptor(descriptor, sourceIdentity, restoreContext = {}) {
    ...
    if (descriptor.source.publicationIdentifier !== String(sourceIdentity?.publicationIdentifier || "").trim() || descriptor.source.publicationModified !== String(sourceIdentity?.publicationModified || "").trim() || descriptor.source.href !== section.href || descriptor.source.cfiBase !== section.cfiBase) {
      return {
        status: "not-applicable",
        reason: "source-mismatch"
      };
    }
    if (descriptor.layout.flow !== "paginated" || descriptor.layout.axis !== "horizontal" || descriptor.layout.direction !== "rtl" || descriptor.layout.writingMode !== "vertical-rl" || descriptor.anchors.readingStart.role !== "reading-start" || descriptor.anchors.readingEndExclusive.role !== "reading-end-exclusive") {
      return {
        status: "not-applicable",
    ...
    if (descriptor.semanticCut) {
      ...
      const resolution = (0, _semanticCut.resolveSemanticCut)(contents.document, view.iframe, descriptor.semanticCut, maxStart, section.cfiBase);
      ...
      this.scrollToLogicalPageInLayout(index, {
        semanticWindowLogicalOffset: maxStart - resolution.physicalStart
      }, true);
      const actual = (0, _semanticCut.captureSemanticCut)(contents.document, view.iframe, section.cfiBase);
    ...
    try {
      ...
      traceTerminal("anchor-resolution-failed", {
        anchorResolution: "invalid"
      });
      return {
        status: "unavailable",
    ...
    const visibleWidth = this.layout.pageWidth || this.layout.width || this.getPageAdvance();
    const maxPhysicalStart = Math.max(0, contentWidth - visibleWidth);
    const startConstraint = (0, _pagination.getVerticalRlSequentialRightBoundaryConstraint)(targetIndex, startAnchorRect.right, 0, 0, 0, 0, 0, 0);
    const candidateSeeds = [baseOffset];
    if (endAnchorRect) {
      candidateSeeds.push(maxPhysicalStart - endAnchorRect.left, maxPhysicalStart - endAnchorRect.right);
    }
    ...
    const canonicalCandidateForClass = classCandidate => {
      const sameClass = observedMatchingCandidates.filter(candidate => candidate.semanticObservation.fingerprint === classCandidate.semanticObservation.fingerprint).sort((left, right) => left.physicalStart - right.physicalStart || left.physicalEnd - right.physicalEnd || left.logicalOffset - right.logicalOffset);
  ...
  scrollToLogicalPageInLayout(pageIndex, options = {}, preserveSourceCut = false) {
    ...
    let logicalOffset = cachedLogicalOffset !== null && (!sequentialBoundaryConstraint || preserveContinuationOffset) ? cachedLogicalOffset : sequentialBoundaryConstraint
    ...
    let snapTolerance = this.getPageSnapTolerance();
    let geometryCandidate = terminalLayout && (0, _terminalContinuation.isVerticalRlTerminalContinuationGeometryCandidate)(terminalLayout, basePageCount, targetIndex, logicalOffset, maxScroll, snapTolerance);
    ...
    if (geometryCandidate) {
      ...
      let terminalTailStateForPromotion = (0, _terminalContinuation.aggregateVerticalRlTerminalTailState)({
      ...
      promotionResult = terminalLayout ? (0, _terminalContinuation.promoteVerticalRlTerminalContinuation)(terminalLayout, basePageCount, targetIndex, logicalOffset, maxScroll, snapTolerance, terminalTailStateForPromotion) : false;
      if (promotionResult) {
        this._verticalRlPageIndexLookupKey = null;
        this.syncVerticalRlViewportClip();
      }
  ...
  queueVerticalRlBoundarySnapRetry(pageIndex, options = {}) {
    ...
    const terminalLayout = this.getVerticalRlTerminalLayout();
    if (terminalLayout && terminalLayout.continuationCount > 0 && targetIndex >= this.getBaseGeometryPageCount() - 1 && this.getCachedVerticalRlLogicalPageOffset(targetIndex, terminalLayout.layoutKey) !== null) {
