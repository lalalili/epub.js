...
class DefaultViewManager {
  ...
  expandVerticalRlLeftMaskToVisibleLine(maskWidths) {
    ...
    try {
      ...
      if (currentPageIndex <= 0) {
        ...
        rightAllowance = (0, _pagination.getVerticalRlPreviousPageRightMask)(visibleWidth, previousPageStep, previousLeftMask, maxMask);
      }
    } catch (error) {
  ...
  getVerticalRlPageOffset(pageIndex, totalPages, maxScroll) {
    ...
    // 末頁釘在內容尾端（maxScroll），不能用步進往後推算——它顯示的是剩餘內容，
    // 與前一頁的間距本來就小於一個完整步進。首頁同理固定為 0。
    if (!this.isRtlVerticalPaginated() || pageIndex <= 0 || pageIndex >= totalPages - 1) {
      return gridOffset;
    }
