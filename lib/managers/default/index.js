  ...
  snapVerticalRlEdgeMaskWidths(widths, maxMask, limits = {}) {
    ...
    const snapLeft = () => {
      ...
      for (const rect of rects) {
        ...
        if (rect.left < boundary && rect.right > boundary) {
          if (!visibleAtNextRight && nearlyVisibleAtNextRight && rect.right - boundary <= edgeTolerance) {
            continue;
          }
    ...
    const snapRight = () => {
      ...
      for (const rect of rects) {
        ...
        if (right > 0 && clippedAtPreviousLeft && rect.right > boundary && rect.left < rawRight) {
      ...
      if (shift !== 0) {
        right = Math.max(0, Math.min(rightMaxMask, right + shift));
      }
