...
class DefaultViewManager {
    ...
    if (continuationCount === 0 && !this._verticalRlTerminalCoverageProjectionInProgress && !this._verticalRlLogicalPageScrollInProgress && this.shouldProjectVerticalRlTerminalContinuation()) {
      ...
      try {
        ...
        if (continuationOffset !== null) {
          ...
          if (snapshot) {
            appendVerticalRlTerminalCoverageTrace("next:terminal-continuation", this.getVerticalRlTerminalCoverageTraceDetail(nominalTotalPages, snapshot, {
  ...
  getVerticalRlTerminalContinuationOffset(snapshot) {
    ...
    let targetRawViewport = (0, _pagination.getVerticalRlRawViewportForOffset)(targetOffset, snapshot.contentWidth, snapshot.visibleWidth);
    let targetCoverage = (0, _pagination.getVerticalRlSemanticCoverage)(snapshot.semanticRects, targetRawViewport, [...snapshot.previousRawViewports, currentViewport], {
      maxScrollHasRoom: false
    });
    if (targetCoverage.uncoveredSemanticRects.length >= snapshot.coverage.uncoveredSemanticRects.length) {
      return null;
    }
