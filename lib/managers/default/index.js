...
class DefaultViewManager {
  ...
  display(section, target) {
    ...
    this.target = target; // Check to make sure the section we want isn't already shown
  ...
  getVerticalRlEdgeMaskWidths() {
    ...
    let hasStructuralGutter = Math.abs(visibleWidth - advance - left) <= 1 && this.getPageBoundaryShift() === 0 && (currentPageIndex <= 0 || Math.abs(previousPageStep - advance) <= 1);
  ...
  getVerticalRlLogicalPageOffsetCacheKey(totalPages, maxScroll) {
    ...
    return [Math.round((Number(totalPages) || 0) * 100) / 100, Math.round((Number(maxScroll) || 0) * 100) / 100, Math.round(contentWidth * 100) / 100, Math.round(visibleWidth * 100) / 100, Math.round(advance * 100) / 100, Math.round((Number(this.layout.edgeGuardPx) || 0) * 100) / 100].join(":");
  }
  ...
  getVerticalRlCleanPageEdgeMaskWidths(advance) {
    ...
    let isSequentialBoundaryPage = !!(this._verticalRlSequentialBoundaryConstraint && this._verticalRlSequentialBoundaryConstraint.pageIndex === currentPageIndex);
  ...
  getLogicalPageStepToNextPage() {
    ...
    if (nextPageIndex === totalPages - 1 && step > advance && this.hasVerticalRlStructuralPageGutter()) {
      return advance;
    }
  ...
  getVerticalRlStructuralEdgeMaskWidthsForLogicalOffset(logicalOffset, contentWidth, visibleWidth) {
    ...
    if (!advance || !contentWidth || !visibleWidth || bleed <= 1) {
      return null;
    }
  ...
  snapVerticalRlLogicalOffsetToTextBoundary(logicalOffset, maxScroll, options = {}) {
    ...
    if (!iframe || !doc || !win || !body || !contentWidth || !visibleWidth || typeof doc.createTreeWalker !== "function") {
      return logicalOffset;
    }

    let cacheKey = [Math.round(logicalOffset * 100) / 100, Math.round(maxScroll * 100) / 100, Math.round(contentWidth * 100) / 100, Math.round(visibleWidth * 100) / 100, this.layout.edgeGuardPx || 0, hasMaxRightBoundary ? Math.round(maxRightBoundary * 100) / 100 : "none", hasPreferredRightBoundary ? Math.round(preferredRightBoundary * 100) / 100 : "none"].join(":");

    if (this._verticalRlBoundarySnapCache && this._verticalRlBoundarySnapCache.key === cacheKey) {
      return this._verticalRlBoundarySnapCache.value;
    }
    ...
    while ((node = walker.nextNode()) && inspected < 1000) {
      let range = doc.createRange();
      range.selectNodeContents(node);
      ...
      for (const rect of Array.from(range.getClientRects())) {
        ...
        let shiftedDistance = Math.min(this.getVerticalRlRectDistanceToLogicalViewport(shiftedLeft, shiftedRight, rightOriginLeft, rightOriginRight), this.getVerticalRlRectDistanceToLogicalViewport(shiftedLeft, shiftedRight, leftOriginLeft, leftOriginRight));
    ...
    let crossingCount = (leftBoundary, rightBoundary) => {
      ...
      for (const rect of rects) {
        if (rect.left < leftBoundary && rect.right > leftBoundary || rect.left < rightBoundary && rect.right > rightBoundary) {
          count += 1;
        }
      }
    ...
    let evaluateBoundaryModel = (leftBoundary, rightBoundary, boundaryOffsetDirection) => {
      ...
      if (initialCrossings > 0) {
        ...
        for (const delta of uniqueCandidates) {
          ...
          if (hasMaxRightBoundary && boundaryOffsetDirection < 0 && contentWidth - snappedOffset > maxRightBoundary + 1) {
            continue;
          }
    ...
    let rightOriginRawLeft = rightOriginRawRight - visibleWidth;
    let rightOriginSnap = evaluateBoundaryModel(rightOriginRawLeft + structuralLeftMask, rightOriginRawRight - structuralRightMask, -1);
    ...
    if (nearestDelta > 0 && boundaryShift > 0 && edgeGuardPx > 0 && structuralBleed > 1 && Math.abs(boundaryShift - edgeGuardPx) <= 1) {
      nearestDelta = Math.min(nearestDelta, Math.max(1, Math.floor(edgeGuardPx / 2)));
    }
  ...
  countPagesWithFractionalTolerance(totalLength, pageLength) {
    if (!Number.isFinite(totalLength) || totalLength <= 0 || !Number.isFinite(pageLength) || pageLength <= 0) {
      return 1;
    }

    let ratio = totalLength / pageLength;
    let rounded = Math.max(1, Math.round(ratio));
    let tolerance = Math.max(1, Math.min(4, pageLength * 0.005));
    ...
    if (this.isRtlVerticalPaginated() && targetIndex > 0) {
      ...
      if (Number.isFinite(forcedRightBoundary) && forcedRightBoundary > 0) {
        ...
        if (currentIndex === targetIndex - 1) {
          ...
          if ((Math.abs(currentOffset - currentGridOffset) > 1 || hasCleanPageLeftMask) && Number.isFinite(maxRightBoundary) && maxRightBoundary > 0) {
            sequentialBoundaryConstraint = {
              pageIndex: targetIndex,
              maxRightBoundary,
  ...
  updateLayout() {
    ...
    if (!this.isPaginated) {
      ...
      this.settings.offset = this.getPageAdvance() / this.layout.divisor; // this.stage.addStyleRules("iframe", [{"margin-right" : this.layout.gap + "px"}]);
