...
class DefaultViewManager {
  ...
  snapVerticalRlLogicalOffsetToTextBoundary(logicalOffset, maxScroll) {
    ...
    let structuralGutterMask = this.hasVerticalRlStructuralPageGutter() ? this.getVerticalRlEdgeMaskWidths() : null;
  ...
  queueVerticalRlBoundarySnapRetry(pageIndex, options = {}) {
    ...
    let retryAttempt = function (attempt) {
      this.waitForVerticalRlLayoutReady().then(function () {
        ...
        let shouldSnapCurrentOffsetToPageGrid = options.useCurrentOffset && this.getPageBoundaryShift() === 0 && this.container && (this.container.clientWidth || 0) - (this.getPageAdvance() || 0) > 1;
