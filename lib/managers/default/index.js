...
class DefaultViewManager {
  ...
  snapVerticalRlEdgeMaskWidths(widths, maxMask, limits = {}) {
    ...
    while ((node = walker.nextNode()) && rects.length < 1000) {
      ...
      for (const rect of Array.from(range.getClientRects())) {
        if (rect.width > 0 && rect.height > 0) {
          if (iframeRect.left < 0 && rect.right >= containerRect.left - maxMask && rect.left <= containerRect.right + maxMask) {
            rects.push({
    ...
    const snapLeft = () => {
      ...
      for (const rect of rects) {
        ...
        } else if (left > 0 && rect.right > rawLeft && rect.left < boundary && rect.right <= boundary && (rawLeftStraddler ? !visibleAtNextRight : !nearlyVisibleAtNextRight) && (!rawLeftStraddler || clippedAtNextRight)) {
  ...
  snapVerticalRlLogicalOffsetToTextBoundary(logicalOffset, maxScroll) {
    ...
    let cacheKey = [Math.round(logicalOffset * 100) / 100, Math.round(maxScroll * 100) / 100, Math.round(contentWidth * 100) / 100, Math.round(visibleWidth * 100) / 100, this.layout.edgeGuardPx || 0].join(":");
  ...
  queueVerticalRlBoundarySnapRetry(pageIndex, options = {}) {
    ...
    let retryAttempt = function (attempt) {
      this.waitForVerticalRlLayoutReady().then(function () {
        ...
        let logicalOffset = options.useCurrentOffset ? Math.max(0, Math.min(maxScroll, currentOffset)) : this.getLogicalOffsetForPageIndex(targetIndex, currentTotalPages, maxScroll);
