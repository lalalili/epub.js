...
class DefaultViewManager {
  ...
  getVerticalRlEdgeMaskWidths() {
    ...
    let hasStructuralGutter = Math.abs(activePageWidth - advance) <= 1 && Math.abs(visibleWidth - advance - left) <= 1 && this.getPageBoundaryShift() === 0;
  ...
  queueVerticalRlBoundarySnapRetry(pageIndex, options = {}) {
    ...
    let retryAttempt = function (attempt) {
      this.waitForVerticalRlLayoutReady().then(function () {
        ...
        let shouldSnapCurrentOffsetToPageGrid = options.useCurrentOffset && this.getPageBoundaryShift() === 0 && this.layout && Math.abs((this.layout.pageWidth || 0) - (this.getPageAdvance() || 0)) <= 1;
