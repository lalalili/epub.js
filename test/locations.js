...
describe('Locations', function() {
	...
	describe('#parse', function() {
		...
		it('creates a fallback location for image-only sections without text nodes', function() {
			...
			assert.ok(result[0].indexOf("epubcfi(/6/2[coverref]!") === 0);
		});

		it('creates a fallback location for empty sections', function() {
			...
			assert.equal(result.length, 1);
			assert.ok(result[0].indexOf("epubcfi(/6/2[emptyref]!") === 0);
		});
